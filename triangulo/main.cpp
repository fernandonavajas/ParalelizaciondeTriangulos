...
void CalcularLargos(int procesadores, int rank){
	int cantidadPorProcesadores=9666/procesadores;
	for (int k=1;k<cantidadPorProcesadores;k++){
		if (rank == k){
			for (int j = cantidadPorProcesadores;j>0;j--){
				int i = (cantidadPorProcesadores*rank)-j;
...
int main(int argc, char *argv[] ){
	clock_t startTime= clock();//tiempo inicial
	MPI_Init(&argc, &argv);
	MPI_Comm_rank(MPI_COMM_WORLD, &rank);
	MPI_Comm_size(MPI_COMM_WORLD, &procesadores);
	MPI_Status status;
	float solicitudDeAngulo=90;//atof(argv[1])
	cout<<" Proyecto ayudantia Computacion paralela "<<endl;
	cout<<endl;
	if (rank==0){
		LeerArchivos();
		MPI_Send(&matriz[27], 9666, MPI_FLOAT[], 1, 0, MPI_COMM_WORLD);
	}
	if (rank!=0){
		MPI_Recv(&matriz[27], 9666, MPI_FLOAT[], 0, 0, MPI_COMM_WORLD, &Stat);
		CalcularLargos(procesadores, rank);
	}
	...
	clock_t endTime  = clock();// tiempo final
	
	cout<<"Se demoró: "<<(double)(endTime - startTime)/CLOCKS_PER_SEC<<" Segundos" <<endl;
return 0 ;
